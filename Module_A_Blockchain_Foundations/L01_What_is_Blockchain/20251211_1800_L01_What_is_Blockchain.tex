...
\begin{itemize}
\item Header $h_i$ with metadata (version, timestamp, nonce, difficulty)
\item Transaction set $T_i = \{tx_1, \ldots, tx_k\}$
\item Hash pointer to previous block: $H(B_{i-1})$
\end{itemize}

\textbf{Integrity Constraint:}
$$\forall i > 0: B_i.\text{prev} = H(B_{i-1})$$

\bottomnote{The hash pointer creates a tamper-evident linked data structure}
\end{frame}

% Block Structure Chart (separate slide for readability)
\begin{frame}[t]{Anatomy of a Blockchain Block}
\begin{center}
\includegraphics[width=0.65\textwidth]{charts/04_block_structure/chart.pdf}
\end{center}

\bottomnote{Bitcoin block header: 80 bytes | Block body: ~1-2 MB of transactions}
\end{frame}
