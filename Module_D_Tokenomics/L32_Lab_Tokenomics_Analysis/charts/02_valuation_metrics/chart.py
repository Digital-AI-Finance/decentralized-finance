...
for bar, val in zip(bars, importance):
    ax.text(bar.get_width() + 1, bar.get_y() + bar.get_height()/2, f'{val}%', ha='left', va='center', fontsize=14, fontweight='bold')

ax.axvline(x=80, color='gray', linestyle='--', alpha=0.5)
ax.text(81, 4.5, 'Essential', fontsize=14, color='gray')

ax.set_xlabel('Importance in Analysis (%)', fontsize=15)
ax.set_xlim(0, 105)
