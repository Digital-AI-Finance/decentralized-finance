...
ax.text(0.25, 0.72, '(Blockchain)', ha='center', fontsize=10, color='#666')
...
for i, item in enumerate(onchain_items):
    ax.text(0.25, 0.60 - i * 0.07, item, ha='center', fontsize=9,
...
ax.text(0.75, 0.72, '(IPFS/Server)', ha='center', fontsize=10, color='#666')
...
for i, item in enumerate(offchain_items):
    ax.text(0.75, 0.60 - i * 0.07, item, ha='center', fontsize=9,
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white', edgecolor=MLORANGE))

# Arrow connecting them
ax.annotate('', xy=(0.55, 0.55), xytext=(0.45, 0.55),
           arrowprops=dict(arrowstyle='->', color='#666', lw=2))
ax.text(0.50, 0.60, 'tokenURI()', ha='center', fontsize=9, fontweight='bold')

# Cost comparison at bottom
ax.text(0.25, 0.20, 'Cost: HIGH', ha='center', fontsize=11,
        fontweight='bold', color=MLRED)
ax.text(0.25, 0.13, '~20,000 gas per 32 bytes', ha='center', fontsize=9, color='#666')

ax.text(0.75, 0.20, 'Cost: LOW', ha='center', fontsize=11,
        fontweight='bold', color=MLGREEN)
ax.text(0.75, 0.13, '~$0.01 per MB (IPFS)', ha='center', fontsize=9, color='#666')
