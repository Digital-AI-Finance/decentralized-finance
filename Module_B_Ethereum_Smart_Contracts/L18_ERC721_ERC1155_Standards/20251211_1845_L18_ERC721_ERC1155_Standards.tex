...
\item Examples: Art, collectibles
\end{itemize}
...
\begin{frame}{Visual Comparison: Fungible vs Non-Fungible}
\begin{center}
\includegraphics[width=0.65\textwidth]{charts/01_fungible_vs_nonfungible/chart.pdf}
\end{center}
\bottomnote{Fungible tokens are interchangeable; NFTs have unique identities and individual value}
\end{frame}
...
\begin{center}
\includegraphics[width=0.70\textwidth]{charts/02_token_standards_comparison/chart.pdf}
\end{center}
\bottomnote{ERC-1155 combines best features of both ERC-20 and ERC-721}
\end{frame}
...
interface IERC721Metadata {
    function name() external view returns (string memory);    // Collection name
    function symbol() external view returns (string memory);  // Collection symbol
...
\begin{frame}{Metadata Storage: On-Chain vs IPFS}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\item Example: Loot (full text on-chain)
\end{itemize}

\vspace{0.2cm}
\textbf{Cost:} Storing 1 KB = approximately 640,000 gas (approximately \$20-100)
\end{column}
\begin{column}{0.48\textwidth}
\textbf{IPFS Storage:}
\begin{itemize}
...
\begin{frame}{Metadata Storage Options Comparison}
\begin{center}
\includegraphics[width=0.70\textwidth]{charts/03_metadata_storage/chart.pdf}
\end{center}
\bottomnote{IPFS offers best balance of cost, decentralization, and permanence}
\end{frame}
...
\item Gateway access: \texttt{https://ipfs.io/ipfs/QmX7H9K2pZ3.../}
\end{itemize}
...
\begin{frame}{NFT Ecosystem Overview}
\begin{center}
\includegraphics[width=0.70\textwidth]{charts/05_nft_ecosystem/chart.pdf}
\end{center}
\bottomnote{Major NFT projects span PFP collections, gaming, utility, and generative art categories}
\end{frame}
...
\begin{center}
\includegraphics[width=0.70\textwidth]{charts/04_batch_gas_savings/chart.pdf}
\end{center}
\bottomnote{ERC-1155 batch transfers reduce gas costs by 50-87\% compared to separate ERC-721 transfers}
\end{frame}

\begin{frame}{Batch Operations: Cost Breakdown}
...
\begin{itemize}
\item Transaction 1: 71,000 gas
\item Transaction 2: 71,000 gas
\item Transaction 3: 71,000 gas
\item Transaction 4: 71,000 gas
\item Transaction 5: 71,000 gas
\end{itemize}

\textbf{Total: 355,000 gas}

\textbf{At 30 Gwei:} 0.01065 ETH
\end{column}
\begin{column}{0.48\textwidth}
...
\item = 60,500 gas total
\end{itemize}

\textbf{Total: 60,500 gas}

\textbf{At 30 Gwei:} 0.001815 ETH
...
\textbf{Challenge:} No on-chain enforcement, purely marketplace-enforced
\end{frame}
...
\item \textbf{Safe Transfers:} \texttt{safeTransferFrom()} prevents token loss by checking recipient compatibility
...
\item \textbf{ERC-1155:} Multi-token standard supporting both fungible and non-fungible tokens
...
\item What are the implications of ENS domains being NFTs for domain squatting?
\end{enumerate}
\end{frame}
