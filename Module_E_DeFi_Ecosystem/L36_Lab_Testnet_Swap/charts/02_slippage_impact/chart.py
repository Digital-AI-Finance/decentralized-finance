...
for bar, sl in zip(bars, slippage):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.3, f'{sl}%', ha='center', va='bottom', fontsize=14, fontweight='bold')

ax.axhline(y=1, color=MLORANGE, linestyle='--', alpha=0.5, linewidth=2)
ax.text(6.5, 1.3, 'Caution', fontsize=14, color=MLORANGE)
ax.axhline(y=5, color=MLRED, linestyle='--', alpha=0.5, linewidth=2)
ax.text(6.5, 5.3, 'High Impact', fontsize=14, color=MLRED)

ax.set_xticks(range(len(trade_sizes)))
ax.set_xticklabels([f'{s} ETH' for s in trade_sizes])
ax.set_xlabel('Trade Size', fontsize=15)
ax.set_ylabel('Price Slippage (%)', fontsize=15)
